...
ROV missions generate a massive, web of multimodal data scattered across different storage architectures and file formats. A single dive typically produces:
...
**Where this tool fits in:** The ROV Lichtblick Pipeline acts as the bridge between raw mission exports and post-dive visualization. It automatically reads, synchronizes, and compiles the diverse data generated by **[GEOMAR ROV systems](https://www.geomar.de/en/st/rov)** into a single, high-performance MCAP file, making your data ready for analysis.

The tool renders +700GB of video and telemetry logs into a relatively small (a few GB) MCAP file in <15 minutes on an M5 MacBook Pro using all hardware acceleration I could find.
...
> **The Practical Setup:** Since storing terabytes of data internally isn't always feasible, I recommend loading your video files onto a fast external Thunderbolt/USB +3 SSD (I use a SanDisk Extreme Pro with 2000mb/s read speed and it works fine) before starting the conversion.
...
With your virtual environment activated, run the web server:
...
-   **The Auto-Tune I/O Choke Tracker:** If you select too many cores, **the system shouldn't crash**. The built-in DIY I/O governor times every read request. If latency spikes > 0.1s, cores will briefly show a `⚠️ I/O Choke` warning and automatically micro-sleep to yield bandwidth, preventing drive failure. At least in theory. 🤞
