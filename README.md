# ROV MCAP Pipeline & Lichtblick Viewer Guide
...
ROV missions generate a massive, chaotic web of multimodal data scattered across different storage architectures and file formats. A single dive typically produces:
 ...
 - **The Solution:** Lichtblick is a powerful, user-friendly visualization suite designed to solve this. It provides an interface that allows you to seamlessly scrub through hours of synchronized video, maps, and time-series sensor data in minutes. **However**, Lichtblick requires all of this data to be packaged in the unified **MCAP file** format. 
...
> **What to Avoid:** Don't run this pipeline directly from a network drive (NAS) or a mechanical spinning backup drive (HDD). The I/O latency will cripple the extraction process and turn a 20-minute job into a multi-day adventure.
...
-   **In-Memory FFmpeg Pipes:** Bypasses disk-write bottlenecks by piping hardware-decoded C-level `ffmpeg` frames directly into Python's active memory.
...
To run this pipeline from scratch, you will need an Apple Silicon Mac (like M5) and a terminal.
...
-   **Zipped Files:** If your Telemetry or CTD data is zipped, the script will automatically extract them into a temporary `./extracted_data` folder in your project directory so your raw drive remains completely untouched.
...
2.  **Video Source Folder:** Point this to your external SSD containing the heavy `.mov`/`.mp4` files. _(Leave blank if videos are in the local data folder)._
...
Choose your parameters carefully based on your hardware (see _Performance Tuning_ below).


![Detected Mission Assets and Navigation Roles & Data Mapping](documentation/detect_mission_data_mapping.png)
**3.2 Detected Mission Assets and Navigation Roles & Data Mapping**
